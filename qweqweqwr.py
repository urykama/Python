...
settings = dict(
    ...
    strategy="BUY",           # Стратегия - SELL (повышение), BUY (понижение)           
    stop_loss_perc = 0.5,       # % оставания от цены
    stop_loss_fixed = 0.0019362,          # Изначальный stop-loss, можно установить руками нужную сумму, потом бот подтянет.
                                # Можно указать 0, тогда бот высчитает, возьмет текущую цену и применит к ней процент
    amount = 0.000011             # Кол-во монет, которое планируем продать (в случае SELL) или купить (в случае BUY)
    ...
    try:
        ...
        if settings['strategy'] == "SELL":
            ...
            if curr_rate > settings['stop_loss_fixed']:
                ...
                res = fCreatOrder (settings['symbol'], settings['strategy'], settings['amount'])
            ...
            if settings['stop_loss_fixed'] > curr_rate :
                ...
                fCreatOrder (settings['symbol'], settings['strategy'], quantity)
